...
La norma dei residui infatti non è altro che il valore all'ottimo, che nel questo caso è dato da \(nr=1.2649e+00\). Si può infatti notare che, nonostante che risolvere il sistema lineare \texttt{Ax=b}
 è equivalente a risolvere il sistema lineare sovradeterminato \texttt{(D*A)x = (D*b)}, ciò non è del tutto vero. Infatti il problema di minimo in questo caso risulterà essere:
 ...
 \(nr = 5.2764e+00\) che risulta essere una soluzione ben peggiore rispetto a quella data da \(x_2\). \\
 ...
 Nel caso invece di D1, possiamo notare che la matrice altro non è che il risultato del prodotto \(\pi*I\).
Il problema in questo caso risulterà essere:
$$\min_{x \in \mathbb{R}^n}\Arrowvert (\pi*A)x-(\pi*b) \Arrowvert_{2}^2 = \Arrowvert \pi*(Ax-b) \Arrowvert_{2}^2 =\pi * \Arrowvert Ax-b \Arrowvert_{2}^2 $$
Infatti le due soluzioni ad eccezzione di qualche decimale coincidono, infatti la norma residua calcolata
ponendo \(x = x_1\) è pari a: \(nr=3.973835306318418e+00\), le cifre inesatte sono dovute all'approssimazione necessaria di \(\pi\), non essendo possible rappresentare il numero interamente in codice macchina.
